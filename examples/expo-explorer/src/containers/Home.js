...
import NpmIconContent from "@svgr-iconkit/fontawesome-brands/icons/regular/npm";
...
export default function Home() {
  ...
  const [isUsingStroke, setUsingStroke] = useState(true)
  ...
  const { iconNames = [], strokeEnabled = false } = iconsetInfo;
        ...
        <Modal isOpen={isSettingOpen} onClose={onSettingClose}>
          <Modal.Content maxWidth="460px">
            ...
            <Modal.Body>
              ...
              <Divider my={2} />
              {!strokeEnabled && <FormControl><FormControl.HelperText>Stroke not supported for this iconset</FormControl.HelperText></FormControl>}
              <FormControl>
                <FormControl.Label>{`Use stroke?`}</FormControl.Label>
                <Checkbox isChecked={isUsingStroke} onChange={() => setUsingStroke(!isUsingStroke)} />
              </FormControl>
              <FormControl>
                <FormControl.Label>{`Stroke width: ${iconStroke}px`}</FormControl.Label>
                <Slider
